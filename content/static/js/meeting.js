$(document).ready(function () {
  var events, stats, tracks, fmp4Data,
  ...
  function loadStream() {
    ...
    var url = '/meetings/' + MEETING_ID + '/playlist.m3u8';
    if (Hls.isSupported()) {
      if (hls) {
        hls.destroy();
        if (hls.bufferTimer) {
  ...
  function updateLevelInfo() {
    ...
    html3 += 'onclick="levelCapping=hls.autoLevelCapping=-1;updateLevelInfo();">auto</button>';
    ...
    for (var i = 0; i < hls.levels.length; i++) {
      ...
      html3 += 'onclick="levelCapping=hls.autoLevelCapping=' + i + ';updateLevelInfo();">' + levelName + '</button>';
