...
__all__ = ('QTechNoNode', 'QTechNoRouterNode', 'QTechNoInputNode')


@bot_manager.register(key=NodeNames.Q_TECH_NO)
class QTechNoNode(BaseNode):
    validators = [
        WorkDay(),
    ...
    def get_messages(self):
        return [_(NodeTranslations.Q_TECH_NO__TXT)]
...
@bot_manager.register(key=NodeNames.Q_TECH_NO_ROUTER)
class QTechNoRouterNode(RouterNode):
    validators = [ChooseNodeOrGoNextNode()]
    handlers = [GoToChosenNodeOrGoToNode(node_key=NodeNames.Q_TECH_NO_INPUT)]


@bot_manager.register(key=NodeNames.Q_TECH_NO_INPUT)
class QTechNoInputNode(BaseEndNode):
    ...
    def get_messages(self):
        return [_(NodeTranslations.Q_TECH_NO__SAVE_OK)]
